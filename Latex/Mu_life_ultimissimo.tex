...
\begin{flushleft}
Sappiamo che, a causa dell'interazione con l'atmosfera, i raggi cosmici che arrivano dallo spazio hanno una composizione differente a livello del mare. In particolare prevalgono i muoni $\mu$, che decadono nel seguente canele con un branching ratio approssimabile al 100$\%$:
...
Per correggere tale effetto abbiamo usato un metodo Monte Carlo per simulare il passaggio dei raggi cosmici attraverso il rivelatore, al fine di ricavare la probabilità che un raggio cosmico passi per una lastra dato il passaggio da altre due (accettanza). 
...
Riportiamo le misure fatte nelle tabelle sottostanti, le colonne sono voltaggio di alimentazione, efficienza, errore sull'efficienza e numero di doppie false:
    ...
    \begin{table}[H]
        \centering
        \resizebox{1\linewidth}{!}{\input{Tabelle/Eff_PMT02}}
        \caption{PMT02}
        \label{tab:PMT02}
    \end{table}    
    \end{minipage}
    ...
    \begin{table}[H]
        \centering
        \resizebox{1\linewidth}{!}{\input{Tabelle/Eff_PMT04}}
        \caption{PMT04}
        \label{tab:PMT04}
    \end{table}    
    \end{minipage}
    ...
    \begin{table}[H]
        \centering
        \resizebox{1\linewidth}{!}{\input{Tabelle/Eff_PMT07}}
        \caption{PMT07}
        \label{tab:PMT07}
    \end{table}
    \end{minipage}
...
\end{figure}
    ...
    \begin{table}[H]
        \centering
        \resizebox{1\linewidth}{!}{\input{Tabelle/Work_tel}}
...
\begin{table}[H]
    ...
    \begin{minipage}{0.5\linewidth}
        \centering
        \resizebox{0.7\linewidth}{!}{\input{Tabelle/Eff_PMT01}}
...
\begin{table}[H]
        \centering
        \resizebox{0.35\linewidth}{!}{\input{Tabelle/work_general}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Immagini/circuit.jpg}
...
\begin{table}[H]
        \centering
        \resizebox{0.5\linewidth}{!}{\input{Tabelle/dati}}
...
Pertanto ci si aspetta: per il grafico \ref{fig:residui} una distribuzione simmetrica attorno a zero; una media campionaria compatibile con zero; una deviazione standard/RMS$\simeq1$, nel grafico vengono poi riportate le seguenti grandezze:
\begin{equation}
...
Nel nostro caso, la distribuzione dei residui di devianza (Fig.~\ref{fig:residui}) risulta ben descritta da una Gaussiana centrata in prossimità di zero: la media campionaria è $<r_D>\simeq -0.035$ e l’RMS $\simeq 1.05$, mentre un fit gaussiano fornisce $\mu_G \simeq (-0.054\pm 0.064)$ e $\sigma_G \simeq (1.02\pm 0.05)$, compatibili entro incertezze con le attese $\mu_G=0$ e $\sigma_G=1$. Ciò supporta l’ipotesi che, nell’intervallo $[t_{\min},t_{\max}]$ adottato e con il modello esponenziale più fondo costante, le discrepanze osservate siano coerenti con fluttuazioni statistiche poissoniane.
...
\resizebox{0.25\linewidth}{!}{\input{Tabelle/parametri}}
...
Per farlo abbiamo assunto nota la vita media del $\mu^{-}$ $\tau_{-}= 2.043\pm0.003 \mu$s, ricavando così quella per il $\mu^{+}$: 
...
\newpage
Sono state quindi realizzate due prese dati (10/12-11/12 e 11/12-15/12) unendo i dataset è seguendo la procedura indicata in questa sezione, è stato quindi realizzato un istogramma dello spettro in energia degli elettroni di decadimento (figura\ref{fig:spettro})
...
\begin{table}[H]
    \begin{minipage}{0.45\linewidth}
        \centering
        \resizebox{0.8\linewidth}{!}{\input{Tabelle/Eff_PMT08}}
        \caption{PMT08}
        \label{tab:PMT08}
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\linewidth}
        \centering
        \resizebox{0.8\linewidth}{!}{\input{Tabelle/Eff_PMT09}}
        \caption{PMT09}
        \label{tab:PMT09}
    \end{minipage}
    \hfill
...
\begin{table}[H]
    \begin{minipage}{0.45\linewidth}
        \centering
        \resizebox{0.8\linewidth}{!}{\input{Tabelle/Eff_PMT10}}
        \caption{PMT10}
        \label{tab:PMT10}
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\linewidth}
        \centering
        \resizebox{0.8\linewidth}{!}{\input{Tabelle/Eff_PMT11}}
        \caption{PMT11}
        \label{tab:PMT11}
    \end{minipage}
    \hfill
\end{table}
\newpage
\section{Stima dell'accettanza dal Monte Carlo}
...
Per ottenere la costante di calibrazione che converte le unità arbitrarie in $eV$ utilizziamo il fatto che la quantità di luce emessa nello scintillatore è proporzionale all'energia rilasciata dalla particella, da cui a sua volta dipenderà l'altezza dei segnali in uscita dal PMT.
