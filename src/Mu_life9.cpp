                 ...
                 double tmax = 20.0)
{
    ...
    char choice_double = 'N';
    std::cin  >> choice_double;

    if (choice_double == 'S' || choice_double == 's') {

        std::cout << "\n[INFO] Fit doppio esponenziale in corso...\n";

        hDecay->GetListOfFunctions()->Clear();

        // ------------------- PARAMETRI FISICI -------------------
        float tau_meno = 2.043;            // vita media mu-
        float tau_plus = 2 * fdTot.tau - tau_meno;

        // stima fondo dal fit precedente
        double bkgGuess = (fExpBkg) ? fExpBkg->GetParameter(2) : 0.0;
        ...
        // ------------------- STIME INIZIALI -------------------
        const double Ntot = hDecay->Integral(
            hDecay->FindBin(tmin),
            hDecay->FindBin(tmax)
        );

        fExpSum->SetParameter(0, 0.5 * Ntot);
        fExpSum->SetParameter(1, 0.5 * Ntot);
        fExpSum->SetParameter(2, bkgGuess);

        // ------------------- FIT -------------------
        hDecay->Fit(fExpSum, "LISR+");
        ...
        double eB_1     = fExpSum->GetParError(2);

        // ------------------- FUNZIONI SINGOLE -------------------
        TF1* fExpMinus = new TF1("fExpMinus",
            Form("%f*exp(-x/%f)", N_minus, tau_meno), tmin, tmax);

        TF1* fExpPlus  = new TF1("fExpPlus",
            Form("%f*exp(-x/%f)", N_plus, tau_plus), tmin, tmax);

        // ------------------- INTEGRALI -------------------
        double N_Im  = fExpMinus->Integral(tmin, tmax);
        double N_Ip  = fExpPlus->Integral(tmin, tmax);

        // ------------------- ABBONDANZE -------------------
        double abb = N_Ip / N_Im ;

        double e_abb = abb*std::sqrt(1/ N_Ip + 1.0 / N_Im);

        // ------------------- OUTPUT -------------------
        std::cout << "\n================ FIT DOPPIO ESPONENZIALE ================\n";
        std::cout << "tau-  = " << tau_meno << " us (fix)\n";
        std::cout << "tau+  = " << tau_plus << " us\n";
        std::cout << "N-    = " << N_minus << " ± " << eN_minus << "\n";
        std::cout << "N+    = " << N_plus  << " ± " << eN_plus  << "\n";
        std::cout << "B     = " << B_1     << " ± " << eB_1     << "\n";
        std::cout << "Abb.  = " << abb << " ± " << e_abb << "\n";
        std::cout << "========================================================\n";

        // =================== RESIDUI DI DEVIANZA + DEVIANZA/ndf + RMS ===================

        std::vector<double> xRes_a;   xRes_a.reserve(hDecay->GetNbinsX());
        std::vector<double> rDev_a;   rDev_a.reserve(hDecay->GetNbinsX());

        double deviance_a = 0.0;
        int    nBinsUsed_a = 0;

        // range bin coerente con tmin/tmax
        int binMin_a = hDecay->FindBin(tmin);
        int binMax_a = hDecay->FindBin(tmax);

        for (int i_a = binMin_a; i_a <= binMax_a; ++i_a) {

            const double ni_a = hDecay->GetBinContent(i_a);
            const double x_a  = hDecay->GetBinCenter(i_a);
            const double bw_a = hDecay->GetBinWidth(i_a);

            // μ_i: atteso dal modello integrato sul bin
            double mui_a = fExpSum->Integral(x_a - bw_a/2.0, x_a + bw_a/2.0);

            // sicurezza numerica
            if (mui_a <= 0.0) continue;

            double ri_a = 0.0;

            if (ni_a > 0.0) {
                const double term_a =
                    2.0 * (ni_a * std::log(ni_a / mui_a) - (ni_a - mui_a));

                ri_a = std::copysign(
                    std::sqrt(std::max(0.0, term_a)),
                    (ni_a - mui_a)
                );
            } else {
                // limite ni = 0
                ri_a = -std::sqrt(2.0 * mui_a);
            }

            xRes_a.push_back(x_a);
            rDev_a.push_back(ri_a);

            deviance_a += ri_a * ri_a;
            nBinsUsed_a++;
        }

        // ndf: bins usati - parametri liberi
        const int nParams_a = fExpSum->GetNpar();   // N-, N+, B
        const int ndf_dev_a = nBinsUsed_a - nParams_a;

        double dev_red_a = (ndf_dev_a > 0) ? (deviance_a / ndf_dev_a) : 0.0;

        // RMS dei residui
        double rms_res_a = 0.0;
        double mean_res_a = 0.0;
        ...
        if (!rDev_a.empty()) {
            ...
            for (double r_a : rDev_a) { 
                sum2_a += r_a * r_a;
                sum_a += r_a;
            }

            rms_res_a = std::sqrt(sum2_a / rDev_a.size());
            mean_res_a = sum_a / rDev_a.size();
        }

        cout << dev_red_a << endl;
        cout << rms_res_a << endl;
        cout << mean_res_a << endl;


                // =================== CANVAS CON DUE PAD ===================
        TCanvas* cFit = new TCanvas("cFit", "Fit doppio esponenziale", 900, 800);

        TH1F* hDecay_points = (TH1F*)hDecay->Clone("hDecay_points");

        TPad* padTop = new TPad("padTop", "fit", 0.0, 0.30, 1.0, 1.0);
        TPad* padBot = new TPad("padBot", "residui", 0.0, 0.0, 1.0, 0.30);

        padTop->SetBottomMargin(0.02);
        padBot->SetTopMargin(0.05);
        ...
        // istogramma "a punti"
        hDecay_points->SetTitle("Fit doppio esponenziale; t [#mus]; Conteggi");
        hDecay_points->GetXaxis()->SetLabelSize(0); // niente numeri asse X sopra
        hDecay_points->Draw("E");

        // curva di fit
        fExpSum->Draw("SAME");

        // (opzionale) stats+fit in box
        ...
        // se vuoi aggiungere A al box stats (se già ti funziona)
        TPaveStats* st = (TPaveStats*)gPad->GetPrimitive("stats");
        if (st) {
            st->AddText(Form("A = %.3f #pm %.3f", abb, e_abb));
            // box un po' più grande (se serve)
            st->SetX1NDC(0.6); st->SetX2NDC(0.88);
            st->SetY1NDC(0.52); st->SetY2NDC(0.90);
            st->SetTextSize(0.040);
        }

        int nBinsRange_a = (binMax_a - binMin_a + 1);   // numero bin nel range

        TPaveText* boxTop_a = new TPaveText(0.3, 0.70, 0.6, 0.90, "NDC");
        boxTop_a->SetFillColor(0);
        boxTop_a->SetBorderSize(1);
        boxTop_a->SetTextFont(42);
        boxTop_a->SetTextSize(0.035);
        boxTop_a->SetTextAlign(12); // left
        boxTop_a->AddText(Form("[%.1f, %.1f] #mus", tmin, tmax));
        boxTop_a->AddText(Form("Bins: %d", nBinsRange_a));
        boxTop_a->Draw();

        // ====== BOX SINISTRO BASSO: dev/ndf + mean + RMS ======
        TPaveText* boxBot_a = new TPaveText(0.3, 0.7, 0.6, 0.52, "NDC");
        boxBot_a->SetFillColor(0);
        boxBot_a->SetBorderSize(1);
        boxBot_a->SetTextFont(42);
        boxBot_a->SetTextSize(0.035);
        boxBot_a->SetTextAlign(12);
        boxBot_a->AddText(Form("D/ndf = %.2f", dev_red_a));
        boxBot_a->AddText(Form("<r> = %.3f  RMS = %.3f", mean_res_a, rms_res_a));
        boxBot_a->Draw();
        ...
        // costruisco il TGraph dei residui (devianza)
        const int nRes_a = (int)rDev_a.size();
        TGraph* gRes_a = new TGraph(nRes_a);

        for (int j_a = 0; j_a < nRes_a; ++j_a) {
            gRes_a->SetPoint(j_a, xRes_a[j_a], rDev_a[j_a]);
        }

        gRes_a->SetMarkerStyle(20);
        gRes_a->SetMarkerSize(0.8);
        gRes_a->SetLineColor(kBlack);

        gRes_a->SetTitle("");
        gRes_a->GetXaxis()->SetTitle("t [#mus]");
        gRes_a->GetYaxis()->SetTitle("Residui devianza");

        // assi leggibili nel pad basso
        gRes_a->GetXaxis()->SetTitleSize(0.12);
        gRes_a->GetXaxis()->SetLabelSize(0.10);
        gRes_a->GetXaxis()->SetTitleOffset(1.0);

        gRes_a->GetYaxis()->SetTitleSize(0.10);
        gRes_a->GetYaxis()->SetLabelSize(0.09);
        gRes_a->GetYaxis()->SetTitleOffset(0.45);
        gRes_a->GetYaxis()->SetNdivisions(505);

        // range verticale (puoi cambiarlo)
        gRes_a->GetYaxis()->SetRangeUser(-4.0, 40.0);

        gRes_a->Draw("AP");

        // linea a zero
        TLine* l0_a = new TLine(tmin, 0.0, tmax, 0.0);
        l0_a->SetLineStyle(2);
        l0_a->Draw("SAME");
