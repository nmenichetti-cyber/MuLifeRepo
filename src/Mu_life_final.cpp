 ...
 * 
 * 
 *  Mu_life_new("/home/luke/Scrivania/Lab_int_fond/Esperienze/Mu_life/dati/FIFO_combined.txt",300 ,0.8, 18.0) 
 * 
 * 
...
const double tick_us    = 0.0048892;                             // 4.98892 ns costante di calibrazione trovata 
...
// =====================================================================
//
// Nota importante sulle cifre significative nel CSV:
// l'utente ha richiesto di evitare "numeri lunghissimi" e di troncare/arrotondare
// a poche cifre significative (es.: 0.0002927676 -> 0.00029). Per default qui
// usiamo 2 cifre significative per TUTTI i campi floating nel CSV.
// Se vuoi più dettaglio (es. per #tau), modifica kSigDigitsCSV.
//
// ---------------------------------------------------------------------

// Confronto robusto per double (utile per filtrare tmin/tmax "nominali" nello scan)
inline bool AlmostEqual(double a, double b, double rel = 1e-12, double abs = 1e-12)
{
...
// Arrotondamento + formattazione a N cifre significative, con scelta automatica
// tra formato "fixed" e "scientific".
// - Ritorna stringa vuota se x è NaN/inf (comodo per CSV).
// - Evita zeri finali inutili in formato fixed.
std::string FormatSig(double x, int sig = 2)
{
    if (!std::isfinite(x)) return "";
    if (x == 0.0) return "0";

    const double ax = std::fabs(x);
    const int exp10 = (int)std::floor(std::log10(ax));
    const double scale = std::pow(10.0, exp10 - sig + 1);

    // "scale" potrebbe diventare 0 per numeri estremi: difesa
    if (!(scale > 0.0) || !std::isfinite(scale)) {
        std::ostringstream oss;
        oss.setf(std::ios::scientific);
        oss << std::setprecision(std::max(0, sig - 1)) << x;
        return oss.str();
    }

    const double xr = std::round(x / scale) * scale;

    // Scegliamo una rappresentazione leggibile:
    // - fixed se l'esponente è "moderato" (tipico per tmin,tmax,tau,dev/ndf, p-values)
    // - scientific per valori molto grandi/piccoli
    const double axr = std::fabs(xr);
    const int exp10r = (axr > 0.0) ? (int)std::floor(std::log10(axr)) : 0;

    std::ostringstream oss;

    if (exp10r >= -6 && exp10r <= 6) {
        const int decimals = std::max(0, sig - exp10r - 1);
        oss.setf(std::ios::fixed);
        oss << std::setprecision(decimals) << xr;

        // trim trailing zeros (e punto finale)
        ...
        oss.setf(std::ios::scientific);
        oss << std::setprecision(std::max(0, sig - 1)) << xr;
        return oss.str();
    }
}

static inline std::string FormatFullDouble(double x)
{
    std::ostringstream oss;
...
bool WriteScanCSV(const std::string& csvName, const std::vector<ScanRow>& rows)
{
    ...
    for (const auto& r : rows) {
        fout << FormatSig(r.tmin,  kSigDigitsCSV) << ","
             << FormatSig(r.tmax,  kSigDigitsCSV) << ","
             << r.nbins << ","
             << FitModelName(r.model) << ","
             << FormatFullDouble(r.tau)  << ","
             << FormatFullDouble(r.etau) << ","
             << FormatSig(r.devOverNdf, kSigDigitsCSV) << ","
             << FormatSig(r.rmsRD,      kSigDigitsCSV) << ","
             << FormatSig(r.nll,        kSigDigitsCSV)
             << "\n";
    }
...
// ---------------------------------------------------------------------
// Grafici richiesti (1D "puliti"):
//   1) τ vs t_min   (t_max, nbins fissi)
//   2) τ vs t_max   (t_min, nbins fissi)
//   3) τ vs nbins   (t_min, t_max fissi)
//   4) dev/ndf vs t_min (t_max, nbins fissi)
//
// Il CSV rimane "completo" (tutte le combinazioni). I grafici 1D si ottengono
// filtrando il sottoinsieme di righe compatibile con una configurazione di riferimento
// (refTmin, refTmax, refNbins, refModel).
// ---------------------------------------------------------------------
void MakeAndSaveGraphs(const std::string& prefix,
                       ...
                       double refTmax,
                       int    refNbins,
                       FitModel refModel = FitModel::ExpBkg,
                       bool alsoMakeGlobalGraphs = true)
{
    gStyle->SetOptStat(0);

    // ROOT file per oggetti (TGraph/TGraphErrors)
    const std::string rootName = prefix + "_scan_graphs.root";
    TFile* froot = new TFile(rootName.c_str(), "RECREATE");

    // -------------------------------
    // (Opzionale) grafici "globali" (scatter su tutte le combinazioni)
    // -------------------------------
    TGraphErrors* gTauVsIdx   = nullptr;
    TGraphErrors* gTauVsTminAll = nullptr;
    TGraphErrors* gTauVsTmaxAll = nullptr;
    TGraphErrors* gTauVsNbinsAll= nullptr;

    if (alsoMakeGlobalGraphs) {
        gTauVsIdx = new TGraphErrors();
        gTauVsIdx->SetName("gTauVsIndex");
        gTauVsIdx->SetTitle("Tau vs indice scan (tutte le combinazioni);indice scan;#tau [#mus]");

        gTauVsTminAll = new TGraphErrors();
        gTauVsTmaxAll = new TGraphErrors();
        gTauVsNbinsAll= new TGraphErrors();

        gTauVsTminAll->SetName("gTauVsTmin_all");
        gTauVsTmaxAll->SetName("gTauVsTmax_all");
        gTauVsNbinsAll->SetName("gTauVsNbins_all");

        gTauVsTminAll->SetTitle("#tau vs t_{min} (tutte le combinazioni);t_{min} [#mus];#tau [#mus]");
        gTauVsTmaxAll->SetTitle("#tau vs t_{max} (tutte le combinazioni);t_{max} [#mus];#tau [#mus]");
        gTauVsNbinsAll->SetTitle("#tau vs nbins (tutte le combinazioni);nbins;#tau [#mus]");

        int ip = 0, itmin = 0, itmax = 0, inb = 0;
        for (const auto& r : rows) {
            ...
            gTauVsTminAll->SetPoint(itmin, r.tmin, r.tau);
            gTauVsTminAll->SetPointError(itmin, 0.0, r.etau);
            itmin++;

            gTauVsTmaxAll->SetPoint(itmax, r.tmax, r.tau);
            gTauVsTmaxAll->SetPointError(itmax, 0.0, r.etau);
            itmax++;

            gTauVsNbinsAll->SetPoint(inb, (double)r.nbins, r.tau);
            gTauVsNbinsAll->SetPointError(inb, 0.0, r.etau);
            inb++;
        }
    }

    // -------------------------------
    // Grafici 1D "puliti" richiesti
    // -------------------------------
    TGraphErrors* gTauVsTminFix  = new TGraphErrors();
    TGraphErrors* gTauVsTmaxFix  = new TGraphErrors();
    TGraphErrors* gTauVsNbinsFix = new TGraphErrors();
    TGraph*       gDevVsTminFix  = new TGraph();

    gTauVsTminFix ->SetName("gTauVsTmin_fixed");
    gTauVsTmaxFix ->SetName("gTauVsTmax_fixed");
    gTauVsNbinsFix->SetName("gTauVsNbins_fixed");
    gDevVsTminFix ->SetName("gDevOverNdfVsTmin_fixed");

    // TITOLI (richiesti in italiano)
    gTauVsTminFix ->SetTitle("Tempo di decadimento stimato #tau in funzione di t_{min} (t_{max} e binning fissi);t_{min} [#mus];#tau [#mus]");
    gTauVsTmaxFix ->SetTitle("Tempo di decadimento stimato #tau in funzione di t_{max} (t_{min} e binning fissi);t_{max} [#mus];#tau [#mus]");
    gTauVsNbinsFix->SetTitle("Tempo di decadimento stimato #tau in funzione del binnaggio (t_{min}, t_{max} fissi);n_{bin};#tau [#mus]");
    gDevVsTminFix ->SetTitle("Devianza/ndf in funzione di t_{min} (t_{max} e binning fissi);t_{min} [#mus];devianza/ndf");

    // Colleziona punti e ordina per ascissa (per rendere i grafici "leggibili")
    struct PTE { double x, y, ey; };
    struct PT  { double x, y; };

    auto FillTauGraph = [&](TGraphErrors* g, const std::vector<PTE>& pts){
        int ip = 0;
        for (const auto& p : pts) {
            g->SetPoint(ip, p.x, p.y);
            g->SetPointError(ip, 0.0, p.ey);
            ip++;
        }
    };
    auto FillGraph = [&](TGraph* g, const std::vector<PT>& pts){
        int ip = 0;
        for (const auto& p : pts) {
            g->SetPoint(ip, p.x, p.y);
            ip++;
    ...
    // τ vs t_min (fissi: tmax=refTmax, nbins=refNbins)
    std::vector<PTE> ptsTauTmin;
    std::vector<PT>  ptsDevTmin;
    for (const auto& r : rows) {
        if (r.model != refModel) continue;
        if (r.nbins != refNbins) continue;
        if (!AlmostEqual(r.tmax, refTmax)) continue;

        if (std::isfinite(r.tau) && std::isfinite(r.etau)) {
            ptsTauTmin.push_back({r.tmin, r.tau, r.etau});
        }
        if (std::isfinite(r.devOverNdf)) {
            ptsDevTmin.push_back({r.tmin, r.devOverNdf});
        }
    }
    std::sort(ptsTauTmin.begin(), ptsTauTmin.end(), [](const PTE& a, const PTE& b){ return a.x < b.x; });
    std::sort(ptsDevTmin.begin(), ptsDevTmin.end(), [](const PT& a, const PT& b){ return a.x < b.x; });
    FillTauGraph(gTauVsTminFix, ptsTauTmin);
    FillGraph(gDevVsTminFix, ptsDevTmin);

    // τ vs t_max (fissi: tmin=refTmin, nbins=refNbins)
    std::vector<PTE> ptsTauTmax;
    for (const auto& r : rows) {
        if (r.model != refModel) continue;
        if (r.nbins != refNbins) continue;
        if (!AlmostEqual(r.tmin, refTmin)) continue;

        if (std::isfinite(r.tau) && std::isfinite(r.etau)) {
            ptsTauTmax.push_back({r.tmax, r.tau, r.etau});
        }
    }
    std::sort(ptsTauTmax.begin(), ptsTauTmax.end(), [](const PTE& a, const PTE& b){ return a.x < b.x; });
    FillTauGraph(gTauVsTmaxFix, ptsTauTmax);

    // τ vs nbins (fissi: tmin=refTmin, tmax=refTmax)
    std::vector<PTE> ptsTauNb;
    for (const auto& r : rows) {
        if (r.model != refModel) continue;
        if (!AlmostEqual(r.tmin, refTmin)) continue;
        if (!AlmostEqual(r.tmax, refTmax)) continue;
        ...
        if (std::isfinite(r.tau) && std::isfinite(r.etau)) {
            ptsTauNb.push_back({(double)r.nbins, r.tau, r.etau});
        }
    }
    std::sort(ptsTauNb.begin(), ptsTauNb.end(), [](const PTE& a, const PTE& b){ return a.x < b.x; });
    FillTauGraph(gTauVsNbinsFix, ptsTauNb);

    // -------------------------------
    // Salvataggio su ROOT file
    // -------------------------------
    froot->cd();

    if (alsoMakeGlobalGraphs) {
        if (gTauVsIdx)      gTauVsIdx->Write();
        if (gTauVsTminAll)  gTauVsTminAll->Write();
        if (gTauVsTmaxAll)  gTauVsTmaxAll->Write();
        if (gTauVsNbinsAll) gTauVsNbinsAll->Write();
    }

    gTauVsTminFix ->Write();
    gTauVsTmaxFix ->Write();
    gTauVsNbinsFix->Write();
    gDevVsTminFix ->Write();
    ...
    // -------------------------------
    // Salvataggio PNG con "box" in alto a destra (richiesto)
    // -------------------------------
    auto AddInfoBoxTopRight = [&](const std::vector<std::string>& lines){
        // Box in coordinate NDC (0..1): in alto a destra
        TPaveText* box = new TPaveText(0.75, 0.75, 0.9, 0.9, "NDC");
        box->SetFillColor(0);
        box->SetBorderSize(1);
        box->SetTextFont(42);
        box->SetTextSize(0.030);
        box->SetTextAlign(12);
        for (const auto& s : lines) box->AddText(s.c_str());
        box->SetBit(kCanDelete); // il canvas se ne prende ownership
        return box;
    };

    auto SaveGraphPNG_E = [&](TGraphErrors* g, const std::string& outPng, const std::vector<std::string>& boxLines){
        if (!g || g->GetN() == 0) {
            std::cerr << "[WARN] Grafico vuoto: " << (g ? g->GetName() : "null") << " (skip PNG)\n";
            return;
        }
        TCanvas* c = new TCanvas(Form("c_%s", g->GetName()), g->GetTitle(), 900, 700);
        g->SetMarkerStyle(20);
        g->Draw("AP");
        AddInfoBoxTopRight(boxLines)->Draw();
        c->SaveAs(outPng.c_str());
        delete c;
    };

    auto SaveGraphPNG = [&](TGraph* g, const std::string& outPng, const std::vector<std::string>& boxLines){
        if (!g || g->GetN() == 0) {
            std::cerr << "[WARN] Grafico vuoto: " << (g ? g->GetName() : "null") << " (skip PNG)\n";
            return;
        }
    ...
    // Linee comuni (NO modello, come richiesto)
    const std::string lineFile = "File: " + prefix;
    const std::string lineFit  = "Fit: L + I + R (Poisson)";

    // τ vs tmin (fissi: tmax, nbins)
    {
        std::vector<std::string> box = {
            //lineFile,
            //lineFit,
            "t_{max} = " + FormatSig(refTmax, 3) + " #mus",
            "n_{bin} = " + std::to_string(refNbins),
            //"Pairing dt: 0.." + FormatSig(FINAL_STOP_MAX_US, 3) + " #mus"
        };
        SaveGraphPNG_E(gTauVsTminFix, prefix + "_tau_vs_tmin_fixed.png", box);
    }

    // τ vs tmax (fissi: tmin, nbins)
    {
        std::vector<std::string> box = {
            //lineFile,
            //lineFit,
            "t_{min} = " + FormatSig(refTmin, 3) + " #mus",
            "n_{bin} = " + std::to_string(refNbins),
            //"Pairing dt: 0.." + FormatSig(FINAL_STOP_MAX_US, 3) + " #mus"
        };
        SaveGraphPNG_E(gTauVsTmaxFix, prefix + "_tau_vs_tmax_fixed.png", box);
    }

    // τ vs nbins (fissi: tmin, tmax)
    {
        std::vector<std::string> box = {
            //lineFile,
            //lineFit,
        ...
        SaveGraphPNG_E(gTauVsNbinsFix, prefix + "_tau_vs_nbins_fixed.png", box);
    }

    // dev/ndf vs tmin (fissi: tmax, nbins)
    {
        std::vector<std::string> box = {
            //lineFile,
            //lineFit,
        ...
        SaveGraphPNG(gDevVsTminFix, prefix + "_devOverNdf_vs_tmin_fixed.png", box);
    }

    // (Opzionale) salva anche i global scatter, ma senza box (per non fare confusione)
    if (alsoMakeGlobalGraphs) {
        auto SaveScatter = [&](TGraphErrors* g, const std::string& outPng){
        ...
        SaveScatter(gTauVsIdx,      prefix + "_tau_vs_index.png");
        SaveScatter(gTauVsTminAll,  prefix + "_tau_vs_tmin_all.png");
        SaveScatter(gTauVsTmaxAll,  prefix + "_tau_vs_tmax_all.png");
        SaveScatter(gTauVsNbinsAll, prefix + "_tau_vs_nbins_all.png");
    }

    // Clean heap
    delete gTauVsTminFix;
    delete gTauVsTmaxFix;
    delete gTauVsNbinsFix;
    delete gDevVsTminFix;

    if (alsoMakeGlobalGraphs) {
        delete gTauVsIdx;
        delete gTauVsTminAll;
        delete gTauVsTmaxAll;
        delete gTauVsNbinsAll;
    }

    // (Opzionale) riassunto START in txt per tracciabilità
...
// Entry point scan
void Mu_life_scan(const char* filename = "FIFOread_combined.txt")
{
    std::cout << "\n============================================\n";
    ...
    std::vector<double> tmaxList  = {10.0,12.0, 15.0,17.0, 18.0, 19.0, 20.0};
    std::vector<int>    nbinsList = {150, 200, 250, 300, 400, 500,1000};
    ...
    // Grafici + ROOT file
    MakeAndSaveGraphs(prefix, rows, sstats,
                      /*refTmin=*/0.8,
                      /*refTmax=*/20.0,
                      /*refNbins=*/300,
                      /*refModel=*/FitModel::ExpBkg,
                      /*alsoMakeGlobalGraphs=*/false);
    std::cout << "[INFO] Grafici 1D (fissi) salvati: " << prefix
              << "_tau_vs_tmin_fixed.png, _tau_vs_tmax_fixed.png, _tau_vs_nbins_fixed.png, _devOverNdf_vs_tmin_fixed.png\n";
    std::cout << "[INFO] ROOT file grafici: " << prefix << "_scan_graphs.root\n";
                 ...
                 double tmax = 20.0)
{
    ...
    // Cleanup/exit interattivo da terminale
    char choice_cleanup = 'N';
    std::cout << "\nEseguire il cleanup degli oggetti (fit, residui, istogrammi)? [S/N] ";
    std::cin  >> choice_cleanup;

    if (choice_cleanup == 'S' || choice_cleanup == 's') {
        delete fExpBkg;
        delete hDevRes;
        delete hDevResDist;
        delete fGaus;
        ...
        std::cout << "[INFO] Cleanup completo eseguito.\n";
    } else {
        std::cout << "[INFO] Cleanup saltato: gli oggetti restano disponibili in sessione ROOT.\n";
    }
